...
exports.joinGameRoomHandler = async (io, socket, payload) => {
  ...
  try {
    ...
    // 게임이 존재하는지 확인
    const { game } = await getActiveRoom(joinGameId, transaction);
    ...
    addPlayerToGameInfo(game_id, userId, nickname);
...
exports.leaveGameRoomHandler = async (io, socket, isManualLeave = false) => {
  ...
  try {
    ...
    // 같은 이름의 방 존재 여부 확인
    const { game } = await getActiveRoom(gameId, transaction);
    if (!game) throw new Error("존재하지 않는 방입니다.");
    ...
    deletePlayerFromGameInfo(gameId, userId);
    // 퇴장이 발생한방 전체 알림(나 제외)
